...
This Git repository contains a Gradle multi-project which consists of 3 sub-projects:
...
==== build the lib and publish it

How to build and publish the `test-context-trace-*.jar` into the Maven Local repository?
...
then you will get the `$TestContextTrace/lib/build/libs/test-context-trace-*.jar` file, which is
exported into the Maven Local repository.

==== aquire the jar into the app project

How to fetch the jar into the `Drivers` folder in the `app` project

You need to read the `app/build.gradle` file to check the version of the `test-context-trace-*.jar` to import
into the `app` project:

[source,groovy]
----
dependencies {
    extraJars 'com.kazurayam:test-context-trace:0.1.1-SNAPSHOT'
}
----
...
Then you will find the `$TestContextTrace/app/Drivers/test-context-trace-*.jar`, which will be
used by Katalon Studio when you run the `$TestContextTrace/app/Test Suites/improved`.

==== publishing the Groovydoc

You can generate the Groovydoc of the lib project and make it visible as a part of the documentation by:

[source,shell]
----
$ cd $TestContextTrace
$ gradle :lib:groovydoc
----
